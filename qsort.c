...
#define QSORT(TYPE, BASE, TOTAL_ELEMS, CMP)				\
{									\
  TYPE *const _base = (BASE);						\
  const unsigned _elems = (TOTAL_ELEMS);				\
  TYPE _hold;								\
									\
  if (_elems > _QSORT_MAX_THRESH) {					\
    TYPE *_lo = _base;							\
    TYPE *_hi = _lo + _elems - 1;					\
    struct {								\
      TYPE *_hi, *_lo;							\
    } _stack[_QSORT_STACK_SIZE], *_top = _stack + 1;			\
									\
    while (_QSORT_STACK_NOT_EMPTY) {					\
      TYPE *_left_ptr, *_right_ptr;					\
									\
  ...
  _jump_over:;								\
      ...
      do {								\
        while ((CMP (_left_ptr, _mid)) < 0)				\
         ++_left_ptr;							\
									\
        while ((CMP (_mid, _right_ptr)) < 0)				\
          --_right_ptr;							\
									\
  ...
  /* Once the BASE array is partially sorted by quicksort the rest	\
     ...
     last element in the array (*not* one beyond it!). */		\
									...
									\
  {									\
    TYPE *const _end_ptr = _base + _elems - 1;				\
    TYPE *_tmp_ptr = _base;						\
    register TYPE *_run_ptr;						\
    TYPE *_thresh;							\
									\
    ...
    while (++_run_ptr <= _end_ptr) {					\
      _tmp_ptr = _run_ptr - 1;						\
      while ((CMP (_run_ptr, _tmp_ptr)) < 0)				\
        --_tmp_ptr;							\
									\
      ++_tmp_ptr;							\
      if (_tmp_ptr != _run_ptr) {					\
        TYPE *_trav = _run_ptr + 1;					\
